 ...
 * @typedef {import('./themeGroup.js').ThemeGroup} ThemeGroup
 */
 ...
 * @returns {Promise<void>} - Resolves when groups are saved successfully, rejects on error
...
/**
 * Loads serialized theme groups from browser storage.
 * Note: Returns plain objects, not ThemeGroup instances.
 * Caller is responsible for deserializing if needed.
 * @returns {Promise<Array<Object>>} Array of serialized theme group objects
 ...
 * @returns {Promise<void>} - Resolves when the active group ID is saved successfully, rejects on error.
